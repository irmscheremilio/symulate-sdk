...
export async function importSchema(options: {
  ...
  try {
    ...
    const outputPath = options.output
      ...
      : path.resolve(process.cwd(), "./src/types/database.ts");
    ...
    console.log(`      id: m.db('users.id'),`);
    console.log(`      name: m.db('users.name', 'German name'), // AI instruction`);
...
function generateTypeScriptTypes(parsedSchema: {
  ...
  for (const [tableName, table] of Object.entries(parsedSchema.tables)) {
    ...
    for (const [columnName, column] of Object.entries(
      ...
      output += `  ${columnName}${optional}: ${tsType}${nullableType};\n`;
    }
